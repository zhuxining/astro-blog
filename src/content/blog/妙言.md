...
pubDate: '2024-05-10T09:26:37+08:00'
...
- 🏂 **妙**：纯本地使用、安全、语法高亮、黑暗模式、源文件保存、国际化、演示模式、PPT 模式、单独编辑模式、文档自动排版、文档导出、内 部跳转、图床、LaTeX、Mermaid、PlantUML
...
2. 可以在 iCloud 或根目录下创建´é一个 `MiaoYan` 的文件夹，打开妙言的设置，将默认存储地址修改成这个。
3. 点击妙言左上角新增文件夹的图标，创建好自己的文档分类文件夹，就可以开始使用了。
4. 同样假如你不习惯默认的字体，可以在设置中修改成其他的正常字体。

[[../Omnivore/好用的AI工具推荐（海外版）|好用的AI工具推荐（海外版）]]
...
- `command + 1`：收起展开目录
- `command + 2`：<mark style="background: #FF5582A6;">收起展开文档列表</mark>
- `command + 3`：<mark style="background: #ADCCFFA6;">切换编辑和预览</mark>
- `command + 4`：==切换到演示模式==

#### 0.2. 文件操作

- `command + n`：新建文档
- `command + r`：<mark class="hltr-yellow">重命名文档</mark>
- `command + d`：复制文档
- `command + o`：单独打开文档
- `command + delete`：**删除文档**
